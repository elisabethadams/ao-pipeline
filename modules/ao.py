...
def objDirByInstr(instr):
	return mainObjectsDir + instr + "/"
...
def koiDir(koi,instr="ARIES"):
	return objDirByInstr(instr)+koi+"/"

def koiFilterDir(koi,filt,instr="ARIES"):
	if ((filt == "692nm") | (filt == "880nm")):
		return koiDir(koi,instr)+filt+"/"
	else:
...
def limMagFile(koi,filt,instr="ARIES"):
	return koiFilterDir(koi,filt,instr)+finalKOIimageFile(koi,filt,instr)+"_lim.tsv"
def compFileSorted(distCutoff=40,suffix=".tsv"):
	return tableDir+"Stars_within_"+str(distCutoff)+suffix

################# AO PROJECT FUNCTIONS ###############

## Get all KOI that have been reduced (in objDirByInstr(instr))
def getAllKOI(instr="ARIES"):
	koiDirStuff=os.listdir(objDirByInstr(instr))
	allKOI = []
...
def getAllObjects(instr="ARIES"):
	objectDirListing=os.listdir(objDirByInstr(instr))
	allObjects = []
...
#### MODIFIED TO WORK FOR ARIES/PHARO/Speckle
def getPlateScaleFromHeader(fitsfile,verbose=False,instrUsed="ARIES"):
	if instrUsed == "ARIES":
		...
		if (cameraMode=="F/15") | (cameraMode=="f/15") | (cameraMode=="f15") | (cameraMode=="F15"):
			...
			plateScale = 0
	elif instrUsed == "PHARO":
		plateScale = 0.0244
	elif instrUsed == "Speckle":
...
#### File locations
def getDataDir(obj):
	settingsDict=readSettingsFile(settingsFile(obj))
	realDataDir = dataDir + settingsDict["Night"] + "/"
###	return settingsDict["Raw_data_dir"] ## NO!
	return realDataDir
...
def readFullCooAllStars(frame,myFWHM, myBkg, myThresh, numXpixels=1024, numYpixels=1024, ignoreEdgePixels=20, interactive=True):
	...
	if len(coords)==0:
		...
		entry = raw_input("Bkg Std. dev. (currently "+str(myBkg)+"): ")
		if entry != "":
			myBkg = eval(entry)
	...
	g.close()
	return True, coords[useStars[0]][0],coords[useStars[0]][1], numXpixels, numYpixels, ignoreEdgePixels
